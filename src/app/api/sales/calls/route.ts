...
export async function GET(req: NextRequest) {
  ...
  let query = supabase
    .from("sales_calls")
    .select("id, sales_lead_id, sales_program_id, status, started_at, ended_at, duration_seconds, end_reason, recording_url, vapi_cost_usd, vapi_cost_breakdown, created_at, sales_lead:sales_leads(id, full_name, first_name, last_name, phone, company_name), sales_program:sales_programs(id, name), analysis:sales_call_analyses(meeting_booked, workshop_accepted, interest_level, call_rating, sentiment, next_action)")
