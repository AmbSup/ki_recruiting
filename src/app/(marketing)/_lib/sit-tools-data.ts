...
Tatsächliche Lösung: Zwei getrennte Tanks, Gerät wechselte in festen Intervallen zwischen zwei Düften. Der Wechsel unterbrach die Geruchsgewöhnung. Half laut Buch, P&Gs Marktanteil in der Kategorie annähernd zu verdoppeln.`,
    ...
    },
    {
      ...
      pitfall:
        "Nicht verwechseln mit reinem Bündeln mehrerer Funktionen nebeneinander wie bei einem Schweizer Taschenmesser oder einer Multifunktionsuhr (jede Funktion bleibt dort für sich allein). Die gewählte Komponente muss eine ECHTE zusätzliche Aufgabe übernehmen. Es gibt 3 Varianten: (a) eine externe Komponente (Kunde, Partner, Umgebung) übernimmt eine Aufgabe, die dein Produkt sonst selbst erledigt; (b) eine interne Komponente bekommt eine komplett neue Zusatzaufgabe; (c) eine interne Komponente übernimmt die Funktion einer externen Komponente (\"stiehlt\" ihr die Aufgabe). Wechsle zwischen allen 3 Varianten, nicht nur der naheliegendsten. WICHTIG zur Komponentenwahl: Prüfe JEDE Komponente aus der Liste einzeln als Kandidatin — auch unscheinbare Nebenkomponenten (Verpackung, Tank, Gehäuse, Anzeige), UND die naheliegendste/zentralste Komponente (z.B. das Kernsystem, Hauptmodell oder den Hauptmechanismus des Produkts). Schließe die zentrale Komponente NIEMALS von vornherein aus, nur weil sie naheliegend wirkt oder der Produktname/die Ausgangslage bereits andeutet, dass genau sie die entscheidende neue Rolle übernehmen könnte — genau DAS ist in der Praxis sehr oft die eigentliche Pointe, kein Ausschlussgrund. Verteile die 5 Vorschläge trotzdem über mehrere unterschiedliche Komponenten (nicht alle an derselben), aber nimm die zentrale Komponente dabei als ernsthaften Kandidaten mit, statt sie zu meiden — reale Fälle kombinieren zudem oft ZWEI UNABHÄNGIGE Aufgabenvereinigungen für zwei getrennte Probleme (Hauptproblem an der zentralen Komponente, ein zweites Problem wie Finanzierung/Wartung/Information an einer unscheinbaren Komponente).",
      fewShotExamples: [
...
Tatsächliche Lösung: Eine Nutzeraktion erfüllte zwei Aufgaben zugleich — Bot-Schutz UND Buch-Digitalisierung. Half u.a. bei der Digitalisierung historischer Zeitungsarchive (New York Times).`,
        `Fall: IKEA – zwei unabhängige Aufgabenvereinigungen an derselben Komponente
Ausgangslage: Möbelhersteller lieferten traditionell fertig montierte Möbel per Spedition aus — das machte Möbel teuer, sperrig zu lagern und zu transportieren.
Komponenten intern: Möbelbausatz, Bauanleitung, Verpackung, Beschläge, Werkzeug-Beilage, Lagerregal.
Komponenten extern (Closed World): Kunde, eigenes Auto/Transporter, eigene Wohnung, Montagewerkzeug zu Hause, Lieferservice, Möbelfabrik.
SIT-Eingriff: ZWEI unabhängige Aufgabenvereinigungen am selben externen Kunden für zwei getrennte Probleme: (1) Der Kunde übernimmt die Montage (Variante a: externe Komponente übernimmt eine Aufgabe, die sonst die Fabrik erledigt) — löst das Kostenproblem der Fertigmontage. (2) Der Kunde übernimmt zusätzlich den Transport nach Hause (ebenfalls Variante a, aber ein KOMPLETT ANDERES Problem: sonst übernimmt das der Lieferservice) — löst das Transportkostenproblem.
Tatsächliche Lösung: Möbel werden flach verpackt und unmontiert verkauft; der Kunde transportiert und baut sie selbst zusammen. Ergebnis: deutlich günstigere, kompaktere, leichter lagerbare Möbel — zwei unabhängige Kostenblöcke gleichzeitig gelöst, nicht nur einer.`,
      ],
      example: {
...
Actual solution: the plug-in air freshener got two separate tanks. The device alternated between two scents (or room scent and Febreze odor control) at fixed intervals. The switching interrupted scent-adaptation. The book credits the product with roughly doubling P&G's market share in the category.`,
    ...
    },
    {
      ...
      pitfall:
        "Don't confuse this with simply bundling several functions side by side, like a Swiss Army knife or a multi-function watch (each function still only does its own original job there). The chosen component must take on a REAL additional job. There are 3 variants: (a) an external component (customer, partner, environment) takes over a task your product would otherwise do itself; (b) an internal component gets a completely new extra job; (c) an internal component takes over the function of an external component (\"steals\" its job). Alternate between all 3 variants, not just the most obvious one. IMPORTANT on component choice: check EVERY component in the list individually as a candidate — both inconspicuous side components (packaging, tank, housing, display) AND the most obvious/central component (e.g. the product's core system, main model, or main mechanism). NEVER rule out the central component just because it seems obvious, or because the product name/situation already hints that exactly this component could take on the decisive new role — that is very often precisely the point in practice, not a reason to exclude it. Still spread the 5 suggestions across several different components (not all on the same one), but seriously include the central component as a candidate instead of avoiding it — real cases also often combine TWO INDEPENDENT task unifications for two separate problems (the main problem on the central component, a second problem like funding/maintenance/information on an inconspicuous one).",
      fewShotExamples: [
...
Actual solution: one user action fulfilled two jobs at once — bot protection AND book digitization. Helped digitize historical archives, including the New York Times.`,
        `Case: IKEA — two independent task unifications on the same component
Situation: Furniture makers traditionally shipped fully assembled furniture via freight carrier — making furniture expensive, bulky to store and to transport.
Internal components: furniture kit, assembly instructions, packaging, fittings, included tools, warehouse shelving.
External components (Closed World): customer, own car/van, own home, home assembly tools, delivery service, furniture factory.
SIT move: TWO independent task unifications on the same external component (the customer), for two separate problems: (1) the customer takes over assembly (variant a: external component takes over a task otherwise done by the factory) — solves the pre-assembly cost problem. (2) the customer also takes over transport home (also variant a, but a COMPLETELY DIFFERENT problem: otherwise the delivery service does that) — solves the transport-cost problem.
Actual solution: furniture is sold flat-packed and unassembled; the customer transports and assembles it themselves. Result: significantly cheaper, more compact, easier-to-store furniture — two independent cost blocks solved at once, not just one.`,
      ],
      example: {
