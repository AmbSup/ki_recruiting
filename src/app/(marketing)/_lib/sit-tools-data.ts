...
Tatsächliche Lösung: Zwei getrennte Tanks, Gerät wechselte in festen Intervallen zwischen zwei Düften. Der Wechsel unterbrach die Geruchsgewöhnung. Half laut Buch, P&Gs Marktanteil in der Kategorie annähernd zu verdoppeln.`,
    ...
    },
    {
      ...
      pitfall:
        "Nicht verwechseln mit reinem Bündeln mehrerer Funktionen nebeneinander wie bei einem Schweizer Taschenmesser oder einer Multifunktionsuhr (jede Funktion bleibt dort für sich allein). Die gewählte Komponente muss eine ECHTE zusätzliche Aufgabe übernehmen. Es gibt 3 Varianten: (a) eine externe Komponente (Kunde, Partner, Umgebung) übernimmt eine Aufgabe, die dein Produkt sonst selbst erledigt; (b) eine interne Komponente bekommt eine komplett neue Zusatzaufgabe; (c) eine interne Komponente übernimmt die Funktion einer externen Komponente (\"stiehlt\" ihr die Aufgabe). Wechsle zwischen allen 3 Varianten, nicht nur der naheliegendsten. WICHTIG: Konzentriere dich nicht nur auf die naheliegendste/zentralste Komponente (z.B. den offensichtlichen Hauptmechanismus oder Antrieb). Reale Fälle kombinieren oft ZWEI UNABHÄNGIGE Aufgabenvereinigungen an unterschiedlichen Komponenten für unterschiedliche Probleme — eine Komponente löst das Hauptproblem, eine andere, unscheinbarere Komponente (z.B. Verpackung, Tank, Gehäuse, Anzeige) löst nebenbei ein zweites, eigenständiges Problem wie Finanzierung, Wartung oder Information. Prüfe deshalb JEDE Komponente aus der Liste einzeln, nicht nur die mit der offensichtlichsten Funktion. Verteile die 5 Vorschläge über möglichst viele unterschiedliche Komponenten, statt mehrfach an derselben naheliegenden Komponente anzusetzen.",
      fewShotExamples: [
...
Actual solution: the plug-in air freshener got two separate tanks. The device alternated between two scents (or room scent and Febreze odor control) at fixed intervals. The switching interrupted scent-adaptation. The book credits the product with roughly doubling P&G's market share in the category.`,
    ...
    },
    {
      ...
      pitfall:
        "Don't confuse this with simply bundling several functions side by side, like a Swiss Army knife or a multi-function watch (each function still only does its own original job there). The chosen component must take on a REAL additional job. There are 3 variants: (a) an external component (customer, partner, environment) takes over a task your product would otherwise do itself; (b) an internal component gets a completely new extra job; (c) an internal component takes over the function of an external component (\"steals\" its job). Alternate between all 3 variants, not just the most obvious one. IMPORTANT: don't fixate on only the most obvious/central component (e.g. the visible main mechanism or drive). Real cases often combine TWO INDEPENDENT task unifications on different components for different problems — one component solves the main problem, another, less obvious component (e.g. packaging, tank, housing, display) solves an entirely separate second problem like funding, maintenance, or information. So check EVERY component in the list individually, not just the one with the most obvious function. Spread the 5 suggestions across as many different components as possible instead of repeatedly targeting the same obvious one.",
      fewShotExamples: [
