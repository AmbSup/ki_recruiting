...
| `sip:neuronic-sales-v2@sip.vapi.ai` | `https://app.neuronic-automation.ai/api/webhook/vapi` | **leer** (dynamic via webhook) | Sales |
...
| Var | Required? | Purpose |
|---|---|---|
| **`VAPI_API_KEY`** | **Erforderlich** für `/api/recovery/recover-transcript` (Backlog-Recovery von Vapi-Calls). Sonst: optional fallback. | Der reguläre POST /call passiert in n8n. Der Recovery-Endpoint im Next.js holt aber per `GET https://api.vapi.ai/call/{id}` mit Bearer-Auth Transcript + Recording nach + persistiert sauber. Setze in Vercel den gleichen Token wie in der n8n-Credential "Vapi API" (ID `gRoTW0Zg4KstZ6AH`). |
